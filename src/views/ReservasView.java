...
public class ReservasView extends JFrame {
	...
	public ReservasView() {
		...
		txtValor.setBounds(95, 328, 160, 33);
		...
		btnsiguiente.addMouseListener(new MouseAdapter() {
			...
			public void mouseClicked(MouseEvent e) {
				if (ReservasView.txtFechaEntrada.getDate() != null && ReservasView.txtFechaSalida.getDate() != null) {		
					RegistroHuesped registro = new RegistroHuesped(newreserva.getId());
	...
	public  void calcular(JDateChooser fechaE, JDateChooser fechaS) {
		if(fechaE.getDate()!=null && fechaS.getDate()!=null) {
			if(fechaE.getDate().after(fechaS.getDate())) {
				JOptionPane.showMessageDialog(null,"la feha de entrada no puede ser posterior  a la salida","error en fecha",
						JOptionPane.ERROR_MESSAGE);
				return;
			}
			Calendar llegada = fechaE.getCalendar();
			Calendar salida = fechaS.getCalendar();
			int dias =-1;
			int precionoche= 35000;
			int valorPagar;
			while(llegada.before(salida)||llegada.equals(salida)) {
				dias++;
			    llegada.add(Calendar.DATE,1);
			}
			valorPagar=dias*precionoche;
			String valor= String.valueOf(valorPagar);
			valorE=valor;
	...
	public void guardarReserva() {
		...
		Reservas newreserva = new Reservas(java.sql.Date.valueOf(fechaE),java.sql.Date.valueOf(fechaS),val,txtFormaPago.getSelectedItem().toString());
	    reservasController.guardar(newreserva);
	    JOptionPane.showMessageDialog(contentPane, "!! Reserva guardada con exito !! id: " + newreserva.getId());
	    
	    RegistroHuesped cliente = new RegistroHuesped(newreserva.getId());
	    System.out.println(newreserva.getId());
	...
	//Código que permite mover la ventana por la pantlla según la posición de "x" y "y"	
